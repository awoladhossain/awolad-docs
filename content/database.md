...
### চ. Enterprise Database Design Additions (বাস্তব প্রোডাকশন প্রজেক্টের আরও ৪টি গুরুত্বপূর্ণ মিসিং কনসেপ্ট)
বাস্তব প্রোডাকশন সিস্টেমে কোটি কোটি ডাটা হ্যান্ডেল এবং ডেটার নিরাপত্তা নিশ্চিত করতে আরও ৪টি অত্যন্ত অ্যাডভান্সড আর্কিটেকচারাল ডিজাইন প্যাটার্ন নিচে বাস্তব কোডসহ ব্যাখ্যা করা হলো:

#### ১. Table Partitioning Strategy (কোটি কোটি ডেটার জন্য টেবিল পার্টিশনিং)
* **সমস্যা:** আমাদের ই-কমার্স সাইটে ২-৩ বছর পর `orders` টেবিলে যদি ১০ কোটির বেশি রো জমা হয়, তখন সাধারণ ইনডেক্স স্ক্যানও ধীরগতির হয়ে যাবে এবং ডিস্ক আইও (Disk I/O) ফেটে যাবে।
* **সমাধান:** আমরা **Range Partitioning** ব্যবহার করে `orders` টেবিলকে প্রতি বছরের জন্য আলাদা আলাদা ফিজিক্যাল সাব-টেবিলে ভাগ করে দেব। কোয়েরি অপ্টিমাইজার তখন কুয়েরির তারিখ দেখে সরাসরি নির্দিষ্ট সাব-টেবিলে চলে যাবে (Partition Pruning), ফলে বাকি টেবিল স্ক্যান করতেই হবে না!
* **DDL কোড (PostgreSQL):**
```sql
-- ১. পার্টিশন মেম্বার হিসেবে মেইন টেবিল তৈরি (created_at কলামের ওপর পার্টিশন করা)
CREATE TABLE orders_partitioned (
    ...
    PRIMARY KEY (id, created_at) -- পার্টিশন কি অবশ্যই প্রাইমারি কি-এর অংশ হতে হবে
) PARTITION BY RANGE (created_at);

-- ২. ২০২৫ সালের জন্য আলাদা ফিজিক্যাল পার্টিশন টেবিল তৈরি
CREATE TABLE orders_y2025 PARTITION OF orders_partitioned
    FOR VALUES FROM ('2025-01-01 00:00:00') TO ('2026-01-01 00:00:00');

-- ৩. ২০২৬ সালের জন্য আলাদা ফিজিক্যাল পার্টিশন টেবিল তৈরি
CREATE TABLE orders_y2026 PARTITION OF orders_partitioned
    FOR VALUES FROM ('2026-01-01 00:00:00') TO ('2027-01-01 00:00:00');
```

#### ২. Defensive Soft Deletes & Partial Indexing (নিরাপদ সফট ডিলিট ও আংশিক ইনডেক্স)
* **সমস্যা:** প্রোডাকশন ডাটাবেজে আমরা কখনই ইউজার বা অর্ডার রেকর্ড পার্মানেন্টলি ডিলিট (Hard Delete) করি না, কারণ এটি অডিট ট্রেইল ও ট্যাক্স রিপোর্টিং ধ্বংস করে। তাই আমরা সফট ডিলিট (`deleted_at TIMESTAMP NULL`) ব্যবহার করি।
* **বিপদ (ডুপ্লিকেট ইমেইল সমস্যা):** সফট ডিলিটের কারণে যখন কোনো ইউজার তার অ্যাকাউন্ট ডিলিট করে দেয়, তখনও তার ইমেইল টেবিলে থেকে যায়। ফলে সেই ইউজার বা অন্য কেউ আর ওই ইমেইল দিয়ে পুনরায় সাইন-আপ করতে পারে না, কারণ সাধারণ `UNIQUE` কনস্ট্রেইন্ট ডুপ্লিকেট ইমেইল রেজিস্টার করতে বাধা দেবে।
* **সমাধান (Partial Indexing):** আমরা এমন একটি ইউনিক ইনডেক্স তৈরি করব যা কেবল সেই সমস্ত রো-কে চেক করবে যারা বর্তমানে ডিলিট হয়নি (`deleted_at IS NULL`):
```sql
-- ক. users টেবিলে সফট ডিলিট কলাম যোগ করা
ALTER TABLE users ADD COLUMN deleted_at TIMESTAMP NULL;

-- খ. ওল্ড গ্লোবাল ইউনিক কনস্ট্রেইন্ট ড্রপ করে Partial Unique Index তৈরি করা
ALTER TABLE users DROP CONSTRAINT users_email_key;

CREATE UNIQUE INDEX idx_users_email_active 
ON users(email) 
WHERE deleted_at IS NULL;
-- 💡 এর ফলে একাধিক সফট ডিলিট হওয়া ইউজার একই ইমেইল দিয়ে টেবিলে থাকতে পারবে, কিন্তু অ্যাক্টিভ ইউজার হিসেবে একই ইমেইল দিয়ে মাত্র একজনই থাকতে পারবে!
```

#### ৩. Replica Lag & Read-Your-Own-Writes Consistency
* **সমস্যা:** যখন ইউজার প্রোফাইল এডিট করে আপডেট বাটন চাপেন, তখন ডেটা মেইন বা রাইট ডাটাবেসে (Primary DB) যায়। আপডেট শেষ হওয়ার পর স্ক্রিন রিফ্রেশ করলে অ্যাপ্লিকেশন যদি রিড রেপ্লিকা (Read Replica) থেকে ডাটা রিড করে, তবে ডাটাবেসের সিঙ্ক হতে কয়েক মিলি-সেকেন্ড দেরি হওয়ার কারণে (Replica Lag) ইউজার দেখতে পাবেন তার আপডেট করা প্রোফাইল পরিবর্তিত হয়নি! এটি ইউজার এক্সপেরিয়েন্স নষ্ট করে।
* **সমাধান (Write-to-Read Pinning):** সিস্টেম আর্কিটেক্টরা এ ক্ষেত্রে **Sticky Connections** মেকানিজম ব্যবহার করেন:
  * যখনই কোনো ইউজার ডাটাবেসে একটি রাইট কোয়েরি (Write/UPDATE) করেন, অ্যাপ্লিকেশন তার সেশন বা কুকিতে একটি টাইমস্ট্যাম্প ফ্লেগ সেট করে দেয় (যেমন: `last_write_at = now()`)।
  * পরবর্তী ১০ সেকেন্ডের মধ্যে ওই ইউজারের করা সমস্ত রিড কোয়েরিও রিড রেপ্লিকায় না পাঠিয়ে সরাসরি প্রাইমারি ডাটাবেসে (Primary DB) পাঠানো হয়। একে বলা হয় **Read-Your-Own-Writes Consistency**।

#### ৪. Row-Level Security (RLS): ডাটাবেজ লেভেলে IDOR প্রটেকশন
* **সমস্যা:** এপিআই ডেভেলপারদের অসাবধানতার কারণে ইউজার ক তার ব্রাউজারে ইউজার খ-এর অর্ডার আইডি দিয়ে হিট করলে অনেক সময় অন্যের অর্ডার লিস্ট দেখতে পান (IDOR Vulnerability)।
* **সমাধান:** আমরা অ্যাপ্লিকেশন লেভেলের ভুলের ওপর ভরসা না করে সরাসরি ডাটাবেজ ইঞ্জিনের কার্নেলে **Row-Level Security (RLS)** চালু করব, যাতে ডাটাবেস নিজেই সুনিশ্চিত করে যে একজন ইউজার কেবল তার নিজের রো-এর ডেটা ছাড়া অন্যের কোনো রো রিড করতে পারবে না!
```sql
-- ক. orders টেবিলে RLS এনাবেল করা
ALTER TABLE orders ENABLE ROW LEVEL SECURITY;

-- খ. পলিসি তৈরি: কারেন্ট অ্যাপ সেশন ইউজারের আইডি এবং অর্ডারের ইউজার আইডি ম্যাচ করতে হবে
CREATE POLICY user_orders_policy ON orders
FOR SELECT
USING (user_id = NULLIF(current_setting('app.current_user_id', true), '')::integer);

-- গ. অ্যাপ্লিকেশন কোড রান করার সময় ডাটাবেস সেশনে কারেন্ট ইউজারের আইডি পুশ করা
-- (প্রতিটি কোয়েরি রান করার আগে বা কানেকশন পুল থেকে সেশন নেওয়ার সময় এটি রান করা হয়):
SET LOCAL app.current_user_id = '42';

-- ঘ. কোয়েরি রান করা (এখন user_id ৪২ কেবল তার নিজের অর্ডারই দেখতে পাবে, অন্য কিছু সিলেক্ট হবেই না!)
SELECT * FROM orders;
```
