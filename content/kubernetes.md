...
## ৯৮. কুবারনেটিস এপিআই ড্রপস্ট্রেট ও থ্রোটলিং মেকানিজম (API Priority & Fairness: Flow Schemas & Priority Levels)

প্রিয় পাঠক, একটি বাস্তব দুর্যোগের কথা কল্পনা করুন। আপনার ক্লাস্টারে একটি থার্ড-পার্টি কাস্টম অপারেটর বা কন্ট্রোলার রান করছিল। হঠাৎ কোনো বাগের কারণে অপারেটরটি ক্ষিপ্ত হয়ে প্রতি সেকেন্ডে এপিআই সার্ভারে লাখ লাখ রিকোয়েস্ট পাঠানো শুরু করল (একটি ভয়ংকর API Request Storm)। 

এর ফলে এপিআই সার্ভারের সিপিইউ এবং র‍্যাম ১০০% ছুঁয়ে গেল। ঠিক এই সময় ক্লাস্টারে বড় ধরণের ক্র্যাশ ঘটল, এবং আপনি অন-কল ইঞ্জিনিয়ার হিসেবে সার্ভারে লগইন করে কোনোমতে একটি `kubectl get pods` কমান্ড দিয়ে অবস্থা বোঝার চেষ্টা করছেন। কিন্তু হাহ! আপনার কমান্ডটি ঝুলে থাকল এবং অবশেষে এপিআই সার্ভার টাইমআউট দিয়ে দিল!

একটি অপারেটরের পাগলামির কারণে সম্পূর্ণ কন্ট্রোল প্লেন ক্র্যাশ করে একজন মানুষের লাইভ ডিবাগিং অপারেশন বন্ধ করে দেওয়ার এই বিপর্যয় ঠেকাতে কুবারনেটিসে ডিফাইন করা হয়েছে—**API Priority & Fairness (APF)**।

### ক. এপিআই রাউন্ডিং ও ফেয়ারনেস পাইপলাইন
APF (v1.20+) প্রবর্তনের আগে, এপিআই সার্ভারের থ্রোটলিং ছিল একটি বোবা ফিল্টার (`max-requests-inflight`)। সে রিকোয়েস্টের গুরুত্ব বুঝত না; ইন-ফ্লাইট রিকোয়েস্টের কোটা ফুরিয়ে গেলে সে সরাসরি নতুন রিকোয়েস্ট ড্রপ করা শুরু করত—তা আপনার অতি জরুরি `kubectl` রিড কমান্ড হোক বা কাস্টম অপারেটরের রিকোয়েস্ট হোক।

APF ইনকামিং রিকোয়েস্টগুলোকে নিখুঁতভাবে দুটি প্রধান পলিসি দিয়ে ফিল্টার করে:
১. **FlowSchemas (ফ্লো-স্কিমা):** এটি ইনকামিং রিকোয়েস্টগুলোকে গ্রুপ বা ক্যাটাগরাইজ করে (যেমন: কে রিকোয়েস্ট পাঠাচ্ছে, কোন namespace থেকে এবং কি ধরণের মেথড)।
২. **PriorityLevelConfigurations (প্রায়োরিটি লেভেল):** এটি প্রতিটি ফ্লো-স্কিমাকে একটি নির্দিষ্ট ও আইসোলেটেড কিউ (Queue) এবং কনকারেন্সি শেয়ার (Concurrency Shares) বরাদ্দ করে।

```
Incoming Requests ===> [ FlowSchemas Filter ]
                             ||
                             v
                 +-----------------------+
                 | Priority Levels Queue |
                 +-----------+-----------+
            /--------/       |       \--------\
           v                 v                 v
     [ system ]       [ leader-election ]   [ workload-low ]
    (Max Shares,      (High Concurrency)     (Queued/Throttled)
   Never Throttled)
```

### খ. এন্টারপ্রাইজ টিউনিং পলিসি
ডিফল্টরূপে কুবারনেটিস `system` এবং `leader-election` প্রায়োরিটি লেভেলকে সর্বোচ্চ কনকারেন্সি শেয়ার দেয়, যাতে ক্লাস্টার চরম ক্র্যাশের মুখে থাকলেও ইন্টারনাল নোড রিনিউ বা লিডার ইলেকশন কোনো অবস্থাতেই থ্রোটল না হয়। 

অন্যদিকে, সাধারণ ইউজার বা থার্ড-পার্টি কাস্টম সোর্স থেকে আসা রিকোয়েস্টগুলোকে `workload-low` বা `catch-all` কিউতে পাঠানো হয়। কোনো রিকোয়েস্টের ঝড় উঠলে এপিআই সার্ভার কেবল ওই স্পেসিফিক কিউটির রিকোয়েস্ট রিজেক্ট বা থ্রোটল করে, বাকি সমস্ত এডমিন কমান্ড এবং নোড হার্টবিট চোখের পলকে স্বাভাবিকভাবে চলতে থাকে!

---

## ৯৯. কুবারনেটিস নোড মেমরি ওএম কিলিং ও ওএস cgroups v2 ওএম কিলার ইন্টিগ্রেশন (Node OOM Killer vs PSI: Pressure Stall Information)

প্রিয় প্রকৌশলী, একটু মনোযোগ দিন। আমরা এর আগে জেনেছি কীভাবে Kubelet নোডের মেমরি ফুরিয়ে গেলে পড উচ্ছেদ (Eviction) করে। কিন্তু কুবারনেটিসের রিকনসিলিয়েশন লুপের বাইরে ওএস কার্নেল লেভেলে কিন্তু এক নির্মম কিলার সর্বদা ওত পেতে থাকে—**Linux Kernel OOM Killer**。

যখন নোডের মেমরি চরম সংকটে পড়ে, লিনাক্স কার্নেল নোডটি সম্পূর্ণ ডেডলক হওয়া থেকে বাঁচাতে যেকোনো মূল্যে মেমরি খালি করার সিদ্ধান্ত নেয়। সে Kubelet-এর অনুমতির অপেক্ষা করে না; কার্নেল সরাসরি তার **Out-Of-Memory (OOM) Killer** অ্যালগরিদম রান করিয়ে প্রসেসগুলোকে খুন করা শুরু করে।

### ক. ওএম স্কোর টিউনিং বনাম কার্নেল আক্রমণ
কার্নেল প্রসেসগুলোর ওএম স্কোর টিউন করে:
* **BestEffort Pods:** `oom_score_adj = 1000` (সবচেয়ে অরক্ষিত, কার্নেল ওএম কিলার অ্যাক্টিভেট হতেই সবার আগে এদের জবাই করে)।
* **Guaranteed Pods:** `oom_score_adj = -997` (সবচেয়ে সুরক্ষিত, কার্নেল এদের ছুঁতেও ভয় পায়)।

কিন্তু একটি মারাত্মক সমস্যা হলো—যদি আপনার Kubelet বা Containerd ডেমনের ওএম স্কোর সঠিকভাবে প্রোটেক্টেড না থাকে, ওএস কার্নেল ভুল করে আপনার কুবারনেটিসের প্রাণ Kubelet প্রসেসটিকেই কিল করে দিতে পারে!

### খ. cgroups v2 এবং PSI (Pressure Stall Information) ইন্টিগ্রেশন
cgroups v2-এর একীভূত আর্কিটেকচারের সুবাদে এখন কুবারনেটিসে (v1.22+) সোয়াপ মেমরি নিরাপদে ব্যবহারের অনুমতি দেওয়া হচ্ছে। Kubelet সরাসরি লিনাক্স কার্নেলের **PSI (Pressure Stall Information)** ব্যবহার করে এই ওএম কিলিং প্রতিরোধ করে।
* PSI ওএস কার্নেলকে প্রতি মিলি-সেকেন্ডে নোটিফাই করে যে সিপিইউ, মেমরি বা আইও ল্যাগ কত পার্সেন্ট রিসোর্স ডেডলকের দিকে যাচ্ছে।
* Kubelet সরাসরি এই PSI ইভেন্ট রিড করে ওএম কিলার ট্রিগার হওয়ার ঠিক আগেই ওএসকে সিগন্যাল দেয় এবং স্বয়ংক্রিয়ভাবে BestEffort পডগুলোকে গ্রেসফুলি উচ্ছেদ করে কার্নেলের রক্তক্ষয়ী ওএম কিলিং যুদ্ধ রুখে দেয়!

---

## ১০০. কুবারনেটিস নোড প্রেশার প্রোটেকশন ও কার্নেল সিসctl টিউনিং (OS Kernel Sysctl Tuning for High-Scale Nodes)

প্রিয় পাঠক, আপনি যখন একটি বিশাল স্কেলের প্রোডাকশন কুবারনেটিস নোড রান করছেন, যেখানে প্রতি নোডে ১১০ বা তার বেশি হাই-থ্রুপুট কন্টেইনার রান করছে, তখন ওএসের ডিফল্ট টিউনিং কিন্তু আপনার সবচেয়ে বড় শত্রু হতে পারে! 

লিনাক্স ওএসের ডিফল্ট কার্নেল ভ্যালুগুলো মূলত স্ট্যান্ডার্ড ডেডিকেটেড সার্ভারের জন্য ডিজাইন করা, যা কুবারনেটিসের মতো অতি-ডাইনামিক মাল্টি-টেন্যান্ট প্ল্যাটফর্মের জন্য একেবারেই অপটিমাইজড নয়। চলুন ওএস কার্নেলের এই নীরব বোতলনেক বা ট্র্যাজেডি প্রতিরোধের মূল চাবিকাঠিগুলো জেনে আসি।

### ক. নোড ক্র্যাশ প্রতিরোধের গোল্ডেন Sysctls
১. **`net.core.somaxconn` (TCP listen backlog limit):** এটি নোডের ওএস কার্নেলের প্রতিটি সকেটের জন্য সর্বোচ্চ পেন্ডিং কানেকশনের সংখ্যা নির্ধারণ করে। ডিফল্ট মান (সাধারণত ১২৮) অত্যন্ত কম! হাই-স্কেলে নোডের CNI যখন হাজার হাজার রিকোয়েস্ট রাউট করে, সকেট কিউ ফুল হয়ে যায় এবং র্যান্ডম TCP Connection Refused এরর দেখা দেয়। এটিকে কমপক্ষে `৩২৭৬৮` এ টিউন করতে হবে।
২. **`net.ipv4.tcp_max_syn_backlog`:** SYN ফ্লাড হামলা প্রতিরোধ ও হাই-কনকারেন্ট কানেকশন হ্যান্ডেল করতে এই কার্নেল সকেট বাফারটি বাড়িয়ে `১৬৩৮৪` করা আবশ্যক।
৩. **`fs.file-max` (Maximum File Descriptors):** সম্পূর্ণ ওএস নোডে একযোগে সর্বোচ্চ কতটি ফাইল ও সকেট ওপেন থাকতে পারবে। এটিকে বাড়িয়ে `২০৯৭১৫২` (২ মিলিয়ন) করা এন্টারপ্রাইজ স্ট্যান্ডার্ড।
৪. **`fs.inotify.max_user_watches`:** নোডের লগ কালেক্টর (যেমন Fluentd) এবং CNI অনবরত ফাইল চেঞ্জ ট্র্যাক করে। এই কার্নেল ওয়াচ লিমিট ফুরিয়ে গেলে নোডের ফাইল চেঞ্জ রিয়েল-টাইমে নোটিফাই হওয়া বন্ধ হয়ে যায়। এটিকে বাড়িয়ে `৫২৪২৮৮` করতে হবে।

### খ. কুবারনেটিসে কার্নেল টিউনিং প্রয়োগ
পড স্পেকে সরাসরি এই কার্নেল প্যারামিটারগুলো সুরক্ষিতভাবে টিউন করতে আমরা `securityContext.sysctls` ব্যবহার করতে পারি:
```yaml
spec:
  securityContext:
    sysctls:
    - name: net.core.somaxconn
      value: "32768"
```
এটি Kubelet-কে নির্দেশ দেয় কার্নেলের নেমস্পেস বাউন্ডারি ব্যবহার করে ওএস লেভেলে ওই স্পেসিফিক পডের কার্নেল নেটওয়ার্ক কার্ডের ক্ষমতা বহুগুণ বাড়িয়ে দিতে!

---

## ১০১. কুবারনেটিস ডিস্ট্রিবিউটেড ট্র্যাকিং ও ওপেনটেলিমետ্রি এপিআই সার্ভার ট্রেসিং (Distributed Tracing in Control Plane: OpenTelemetry API Server Internals)

প্রিয় রিডার, আপনি যখনই একটি অ্যাপ্লিকেশন ডিপ্লয় করার জন্য `kubectl apply -f app.yaml` রান করেন, আপনার রিকোয়েস্টটি চোখের পলকে এপিআই সার্ভার পার হয়ে, মিউটেটিং ও ভ্যালিডেটিং ওয়েবহুক ঘুরে, etcd-তে সেভ হয়ে, শিডিউলার দ্বারা নোড লক হয়ে, অবশেষে Kubelet-এর কার্নেল সকেটে গিয়ে পৌঁছায়। 

কিন্তু যদি কোনোদিন আপনার এই সম্পূর্ণ ডিপ্লয়মেন্ট লাইফসাইকেলটি স্বাভাবিক ০.৫ সেকেন্ডের পরিবর্তে ১৫ সেকেন্ড সময় নেয়, আপনি কীভাবে নিশ্চিত হবেন যে ঠিক কোন কম্পোনেন্ট বা কোন কাস্টম ওয়েবহুক ল্যাগ তৈরি করছে?

চলুন কন্ট্রোল প্লেনের ভেতরের এই অন্ধকূপের আলো—**OpenTelemetry Distributed Tracing**-এর আর্কিটেকচার দেখে আসি।

### ক. কন্ট্রোল প্লেন ট্রেস প্রোপাগেশন
আধুনিক কুবারনেটিসে (v1.22+) এপিআই সার্ভারের ভেতরে সরাসরি OpenTelemetry (OTel) ট্রেসিং ইঞ্জিন ইনজেক্ট করা হয়েছে।
* **Trace Context Propagation:** এপিআই সার্ভার যখনই কোনো HTTP রিকোয়েস্ট পায়, সে একটি গ্লোবাল **Trace ID** জেনারেট করে। এই আইডিটি এপিআই সার্ভার থেকে শুরু করে কাস্টম অ্যাডমিশন ওয়েবহুক এবং etcd ডেটাবেস লেনদেন পর্যন্ত ট্রাভেল করে।

```
Client Request ===> [ API Server ] ======(W3C Context)======> [ Mutating Webhook ]
                          |                                            |
...
### খ. লাইভ পারফরম্যান্স ডিবাগিং
OTel ট্রেসিং ইনেবল করলে, এপিআই সার্ভার প্রতিটি রিকোয়েস্টের একটি ফিজিক্যাল JSON ট্রেস স্প্যান এক্সপোর্ট করে (যেমন Jaeger বা Zipkin-এ)। আপনি ভিজ্যুয়াল ড্যাশবোর্ডে গিয়ে দেখতে পাবেন:
- `apiserver_request_duration_seconds` মেট্রিক্সের বাইরেও, কাস্টম ইমেজ ইনজেক্টর মিউটেটিং ওয়েবহুকটি আসলে ২.৫ সেকেন্ড সময় নষ্ট করেছে! 
কন্ট্রোল প্লেনের এই নিখুঁত ভিজিবিলিটি আপনাকে এন্টারপ্রাইজ স্কেলে কন্ট্রোল প্লেন বটলনেক সনাক্তকরণে এক অসাধারণ পরাশক্তি এনে দেয়।

---

## ১০২. কুবারনেটিস নেটওয়ার্ক পলিসি কার্নেল আইপিটেবিলস বনাম ইবিপিএফ ফাস্টপাথ (Network Policy enforcement: iptables chains vs eBPF TC BPF filter fastpath)

প্রিয় পাঠক, আপনি নিশ্চয়ই আপনার কুবারনেটিস ক্লাস্টারে Pod-to-Pod সিকিউরিটি লক করার জন্য নেটওয়ার্ক পলিসি (Network Policies) ডিফাইন করেছেন। কিন্তু ওএস কার্নেল লেভেলে কীভাবে একটি নেটওয়ার্ক প্যাকেটকে ফিজিক্যালি ব্লক বা অ্যালাউ করা হয়, এবং এর পেছনে ওএসের মেমরির ওপরে কী বিশাল যুদ্ধ চলে—তা কি কখনো ভেবে দেখেছেন?

চলুন লিনাক্স কার্নেলের দুটি পরস্পরবিরোধী নেটওয়ার্কিং প্রযুক্তির দ্বৈরথ দেখে আসি: **iptables** বনাম **eBPF ফাস্টপাথ**।

### ক. iptables-এর লিনিয়ার ট্র্যাজেডি (The O(N) Tragedy)
Kube-proxy বা ট্র্যাডিশনাল CNI (যেমন Calico iptables মোডে) নেটওয়ার্ক পলিসিগুলো ওএস কার্নেলের নেটফিল্টার (Netfilter) টেবিলের বিশাল **iptables chains**-এ রূপান্তর করে।
* যখন নোডে ১০০০টি পড এবং শত শত পলিসি থাকে, কার্নেলের কাছে আসা প্রতিটি সিঙ্গেল প্যাকেটকে এই লম্বা লাইনের প্রতিটা রুল এক এক করে ওপর থেকে নিচে চেক করে সিদ্ধান্ত নিতে হয়।
* প্যাকেটের গতি যদি ওএসে O(N) লিনিয়ার টাইমে ভ্যালিডেট হতে হয়, তবে হাই-থ্রুপুট নেটওয়ার্কিংয়ের সময় নোডের সিপিইউ ১০০% পিন্ড হয়ে যায় এবং ল্যাটেন্সি কয়েক মিলি-সেকেন্ড বেড়ে যায়!

### খ. eBPF (Cilium/TC BPF) ফাস্টপাথ (The O(1) Absolute Speed)
অন্যদিকে, আধুনিক eBPF-ভিত্তিক CNI (যেমন Cilium) ওএসের নেটফিল্টার বা iptables সম্পূর্ণ বাইপাস করে। 
* সে সরাসরি লিনাক্স কার্নেলের **Traffic Control (tc)** সাবসিস্টেম বা সরাসরি নেটওয়ার্ক কার্ডের ড্রাইভার লেভেলে (**XDP - eXpress Data Path**) একটি BPF কোড ব্লক অ্যাটাচ করে দেয়।
* BPF কোডটি কার্নেল মেমরিতে থাকা ওএম-ম্যাপ (`BPF Maps`) থেকে **O(1)** টাইমে চোখের পলকে পলিসি চেক করে প্যাকেটটি অ্যালাউ বা ড্রপ করে দেয়। 

```
[ iptables Mode ]
Packet ---> Netfilter Stack ---> Rule 1 ---> Rule 2 ... ---> Rule N ---> Pod (O(N) CPU Starvation)

[ eBPF TC Mode ]
Packet ===(XDP Driver Bypass)===> BPF Map Lookup [O(1)] ===> Pod (Absolute Fastpath, Zero Overhead)
```

এই প্রযুক্তিটি নোডের কার্নেল ল্যাটেন্সিকে शून्यে নামিয়ে আনে এবং প্রতি সেকেন্ডে মিলিয়ন ট্রাফিক হ্যান্ডেল করার অতিমানবীয় শক্তি দেয়!

---

## ১০৩. কুবারনেটিস ভলিউম প্রটেকশন ও কার্নেল VFS ডেডলক ট্র্যাজেডি (Volume Locks & Kernel VFS Deadlocks)

প্রিয় প্রকৌশলী, আজকের এই চ্যাপ্টারে আমরা ক্লাউড নেটিভের সবচেয়ে কুখ্যাত এবং ভীতিজনক সমস্যাটি নিয়ে কথা বলব—যা প্রতিটি সিস্টেম অ্যাডমিনিস্ট্রেটরকে জীবনে অন্তত একবার রাতে ঘুমোতে দেয়নি। 

আপনি আপনার একটি স্টেটফুল পড ডিলিট করার কমান্ড দিয়েছেন। কিন্তু পডটি চিরকালের জন্য **`Terminating`** স্টেটে ঝুলে থাকল! আপনি `kubectl delete pod --force --grace-period=0` দিলেন, কিন্তু তাও পডটি মরল না। 

কেন কুবারনেটিসের মতো একটি অল-পাওয়ারফুল অর্কেস্ট্রেটর একটি সাধারণ পডকে নোড থেকে মুছে ফেলতে পারছে না? এর মূল অপরাধী কুবারনেটিস নয়; লিনাক্স কার্নেলের **VFS (Virtual File System) Deadlock**!

### ক. কার্নেল D-State বা আন-ইন্টারাপ্টিবল স্লিপ ট্র্যাজেডি
যখন পডটি কোনো রিমোট নেটওয়ার্ক ভলিউম (যেমন NFS, iSCSI, বা AWS EBS) মাউন্ট করে রান করছিল এবং হঠাৎ ওই আন্ডারলায়িং স্টোরেজ নেটওয়ার্ক বা সকেট ব্লক হয়ে ড্রপ করে:
* কন্টেইনারের ভেতরের ডাটাবেস প্রসেসটি যখন ডিস্কে কোনো ডাটা রাইট করতে যায়, লিনাক্স কার্নেল ফাইল সিস্টেম স্তরে ড্রাইভ লক করে অপেক্ষা করতে থাকে।
* কার্নেল প্রসেসটিকে **`D-State` (Uninterruptible Sleep)**-এ পাঠিয়ে দেয়। লিনাক্স ওএসের নিয়ম অনুযায়ী, `D-State` এ থাকা কোনো প্রসেসকে স্বয়ং ঈশ্বর বা `SIGKILL (Signal 9)` সিগন্যাল দিয়েও কিল করা সম্ভব নয়! Kubelet যখন কন্টেইনার উচ্ছেদ বা আনমাউন্ট করতে যায়, VFS লেয়ারটি ব্লক হয়ে Kubelet-এর থ্রেডকেও চিরতরে স্তব্ধ করে দেয়।

### খ. লাইভ ওএস রিকভারি প্রটোকল
এই ভয়ংকর নোড লকআপ থেকে ক্লাস্টারকে বাঁচাতে নিচের প্রোডাকশন ট্রাভলশুটিং প্রটোকল অনুসরণ করতে হবে:
১. **D-State প্রসেস সনাক্তকরণ:** নোডের ভেতরে ঢুকে কার্নেল প্রসেস লিস্ট থেকে D-State প্রসেসটি খুঁজে বের করুন:
   ```bash
   ps aux | awk '$8 ~ /D/'
   ```
২. **লেজি আনমাউন্ট (Lazy Unmount):** Kubelet-কে লক থেকে মুক্ত করতে কার্নেল ফাইলসিস্টেমের মাউন্ট পয়েন্টকে জোরপূর্বক বিচ্ছিন্ন বা লেজি আনমাউন্ট করুন:
   ```bash
   umount -f -l /var/lib/kubelet/pods/<POD_UID>/volumes/kubernetes.io~nfs/<VOLUME_NAME>
   ```
এই `lazy unmount` কমান্ডটি ওএস কার্নেল লেভেলে মাউন্ট ট্রি থেকেক্লিয়ার করে ডেড নেটওয়ার্ক ডিস্কটিকে তৎক্ষণাৎ বিচ্ছিন্ন করে দেয়, যার ফলে Kubelet চোখের পলকে পডটিকে সম্পূর্ণ উচ্ছেদ করে নোডটিকে ওএস ডেডলকের হাত থেকে রক্ষা করতে পারে!

---

## ১০৪. শীর্ষ ২০টি সিনিয়র-লেভেল কুবারনেটিস ইন্টারভিউ প্রশ্নোত্তর (Top 20 Systems Q&A)
