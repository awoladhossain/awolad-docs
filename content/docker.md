...
## ৩৬. High-Value Scenario-Based Interview Q&A (সিনারিও-ভিত্তিক প্রশ্ন ও সমাধান)

বাস্তব জীবনের প্রোডাকশন ইস্যু সমাধান এবং হাই-লেভেল সিস্টেম ডিজাইন ও ডেভওপস ইন্টারভিউয়ের মুখোমুখি হওয়ার জন্য টপ ২০টি সিনারিও-ভিত্তিক প্রশ্নোত্তর নিচে বিস্তারিতভাবে আলোচনা করা হলো:

### Q1: কন্টেইনারটি মাঝেমধ্যে কোনো মেসেজ ছাড়াই হঠাৎ ক্র্যাশ করছে এবং `exit code 137` দেখাচ্ছে। এর কারণ কী এবং কীভাবে ডায়াগনসিস ও সমাধান করবেন?
* **সিনারিও:** ব্যাকএন্ড কন্টেইনার সচল থাকার কিছুক্ষণ পর হঠাৎ বন্ধ হয়ে যায়। `docker ps -a` দিয়ে দেখলে তার স্ট্যাটাস `Exited (137)` শো করে।
* **কার্নেল লেভেল কারণ:** ডকার কন্টেইনারে যখন তার জন্য বরাদ্দকৃত মেমরি লিমিট (cgroups memory limit) অতিক্রম করে ফেলে, তখন হোস্টের লিনাক্স কার্নেলের **Out-Of-Memory (OOM) Killer** কন্টেইনারের মেইন প্রসেসটিকে (PID 1) `SIGKILL` (signal 9) দিয়ে মেরে ফেলে। ডকারে `128 + 9 = 137` এক্সিট কোড OOM কিলিং নির্দেশ করে।
* **ডায়াগনসিস:** 
  * হোস্টের কার্নেল রিং বাফার চেক করুন: `dmesg -T | grep -i oom`
  * কন্টেইনারের মেটাডেটা চেক করুন: `docker inspect <container_id> --format='{{.State.OOMKilled}}'` (এটি `true` রিটার্ন করবে)।
* **সমাধান:**
  1. ডকার কম্পোজ বা কুবারনেটিসে মেমরি লিমিট বাড়িয়ে দিন (`mem_limit` বা `limits.memory` বাড়ান)।
  2. নোড প্রসেসের ক্ষেত্রে মেমরি লিমিট সেট করুন: `node --max-old-space-size=4096 dist/main.js`
  3. অ্যাপ্লিকেশনের মেমরি লিক (Memory Leak) পরীক্ষা করতে Chrome DevTools বা Profiler ব্যবহার করুন।

---

### Q2: আপনার ব্যাকএন্ড এপিআই সার্ভিসের সিপিইউ ব্যবহার কম (যেমন ১০-২০%), তবুও ইউজাররা ল্যাটেন্সি স্পাইক ও স্লো-রেসপন্স ফেস করছেন। কার্নেল লেভেলে এর কারণ কী হতে পারে?
* **সিনারিও:** কন্টেইনার মেমরি ও সিপিইউ ইউটিলাইজেশন গ্রাফে সবকিছু গ্রিন দেখাচ্ছে, কিন্তু এপিআই রেসপন্স টাইম হঠাৎ ১০০ms থেকে স্পাইক করে ৫০০০ms হয়ে যাচ্ছে।
* **কার্নেল লেভেল কারণ:** এটি **CPU Throttling** এর একটি ক্লাসিক উদাহরণ। ডকার cgroups v2-এর **CFS (Completely Fair Scheduler) Bandwidth Control** মেকানিজম ব্যবহার করে। আপনি যদি আপনার কন্টেইনারে হার্ড লিমিট হিসেবে `cpus: 0.5` বা `cpu-quota` সেট করে রাখেন এবং কন্টেইনারের থ্রেডগুলো তার বরাদ্দকৃত কোটা পিরিয়ডের (সাধারণত ১০০ms) শুরুতেই শেষ করে ফেলে, তবে কার্নেল সেই পিরিয়ডের বাকি সময় কন্টেইনারটিকে সম্পূর্ণরূপে থ্রোটল (স্থগিত) করে রাখে। এর ফলে সিপিইউ ইউটিলাইজেশন কম দেখালেও প্রসেসটি রেসপন্স করতে পারে না।
* **ডায়াগনসিস:** 
  * কন্টেইনারের কার্নেল স্ট্যাটিসটিক্স রিড করুন:
    `cat /sys/fs/cgroup/cpu/cpu.stat` (cgroups v1) অথবা `cat /sys/fs/cgroup/cpu.stat` (cgroups v2)। সেখানে `nr_throttled` এবং `throttled_usec` ভ্যালুগুলো যদি দ্রুত বাড়তে থাকে, তবে বুঝবেন প্রসেস থ্রোটল হচ্ছে।
* **সমাধান:**
  1. প্রোডাকশনে কন্টেইনারে খুব বেশি টাইট বা হার্ড সিপিইউ লিমিট সেট করা থেকে বিরত থাকুন।
  2. কুবারনেটিসে `CPU Limits` রিমুভ করে শুধুমাত্র `CPU Requests` ব্যবহার করুন (যেটি নোডের খালি সিপিইউ ডিস্ট্রিবিউট করতে কার্নেল শেয়ার ব্যবহার করে)।
  3. ডকার কম্পোজে `cpu-period` এবং `cpu-quota` টিউন করে কোটা উন্ডো বাড়ান।

---

### Q3: কন্টেইনারের ভেতরে একাধিক জোম্বি প্রসেস (Zombie Processes) জমা হয়ে হোস্টের প্রসেস টেবিল শেষ করে দিচ্ছে। এর কারণ ও সমাধান কী?
* **সিনারিও:** কন্টেইনারে ঢোকার পর `ps aux` দিলে অনেকগুলো প্রসেসের পাশে `<defunct>` বা জোম্বি স্ট্যাটাস দেখা যাচ্ছে এবং নতুন কন্টেইনার রান হতে পারছে না।
* **কার্নেল লেভেল কারণ:** লিনাক্স ওএসে যেকোনো চাইল্ড প্রসেস যখন তার কাজ শেষ করে এক্সিট করে, তখন সে তার এক্সিট স্ট্যাটাস তার প্যারেন্ট প্রসেসকে পাঠায়। প্যারেন্ট যদি `wait()` বা `waitpid()` সিস্টেম কল রান করে সেটি রিসিভ না করে, তবে চাইল্ড প্রসেসটি প্রসেস টেবিলে জোম্বি হিসেবে থেকে যায়। কন্টেইনারের মেইন এন্ট্রি পয়েন্ট প্রসেসটি (PID 1, যেমন Node.js বা Python) সাধারণত একটি ফুল-ফিচার্ড ওএস ইনিট সিস্টেমের মতো ডিজাইন করা থাকে না, ফলে চাইল্ড প্রসেস রিড করার মেকানিজম (Zombie reaping) তাদের থাকে না।
* **সমাধান:**
  1. ডকার রান করার সময় `--init` ফ্ল্যাগ ব্যবহার করুন: `docker run --init -d my-image` (এটি কন্টেইনারের ভেতর লাইটওয়েট **Tini** ইনিট প্রসেস ঢুকিয়ে দেয় যা জোম্বি ক্লিয়ার করে)।
  2. Dockerfile-এ এন্ট্রি পয়েন্ট হিসেবে Tini বা `dumb-init` ব্যবহার করুন:
     ```dockerfile
...
### Q4: লোকাল ম্যাকবুক (Apple Silicon M1/M2) থেকে ডকার ইমেজ বিল্ড করে ইন্টেল সার্ভারে ডেপ্লয় করলেই `exec user process caused: exec format error` দেখায়। কেন এমন হয় এবং এটি কীভাবে স্থায়ীভাবে সমাধান করবেন?
* **সিনারিও:** লোকাল মেশিনে ইমেজটি চমৎকার চলছে, কিন্তু প্রোডাকশন সার্ভারে ডেপ্লয় করা মাত্রই স্টার্ট না হয়ে ক্র্যাশ করছে।
* **কার্নেল লেভেল কারণ:** এটি সিপিইউ আর্কিটেকচার মিসম্যাচ। Apple Silicon মূলত **ARM64** আর্কিটেকচারের সিপিইউ ব্যবহার করে, কিন্তু অধিকাংশ প্রোডাকশন সার্ভার **AMD64 (x86_64)** আর্কিটেকচারের ইন্টেল বা এএমডি প্রসেসর ব্যবহার করে। আর্কিটেকচার না মিলিয়ে কম্পাইল করা বাইনারি কার্নেল এক্সিকিউট করতে পারে না, ফলে কার্নেল `exec format error` প্রদান করে।
* **সমাধান:**
  1. ডকার প্লাগইন **Docker Buildx** ব্যবহার করে লোকাল মেশিনেই মাল্টি-আর্কিটেকচার ইমেজ বিল্ড করুন:
     ```bash
     docker buildx create --use
     docker buildx build --platform linux/amd64,linux/arm64 -t username/my-app:v1 --push .
     ```
  2. ডকার রেজিস্ট্রি (যেমন Docker Hub) অটোমেটিকালি সঠিক আর্কিটেকচারের ইমেজটি ডেস্টিনেশন সার্ভারে ডেলিভার করবে।

---

### Q5: ডকার হোস্ট সার্ভারের ডিস্ক স্পেস হঠাৎ শেষ হয়ে গেছে। কীভাবে নিরাপদে অপ্রয়োজনীয় ডকার ফাইল ডিলিট করে ডিস্ক মেমরি পুনরুদ্ধার করবেন?
* **সিনারিও:** ডিস্ক ফুল হয়ে যাওয়ায় সার্ভারের ডাটাবেস রাইট এরর দিচ্ছে এবং ডকার কন্টেইনারগুলো বন্ধ হয়ে যাচ্ছে।
* **কারণ:** ডকারে প্রতিনিয়ত পুরানো ইমেজ আপডেট, রান না হওয়া কন্টেইনারের ডেটা, ডকার লগ ফাইলের অনিয়ন্ত্রিত সাইজ বৃদ্ধি এবং ক্যাশ মেমরি জমে ড্রাইভ ব্লক হয়ে যায়।
* **সমাধান:**
  1. বর্তমান ডকার ডিস্ক স্পেস ব্যবহার যাচাই করুন: `docker system df`
  2. এক ক্লিকে স্টপ হয়ে থাকা কন্টেইনার, আনট্যাগড ইমেজ, ক্যাশ এবং অব্যবহৃত নেটওয়ার্ক ডিলিট করুন:
     `docker system prune -a --volumes -f` (সতর্কতা: এটি অল রানিং কন্টেইনার বাদে বাকি সব ক্লিন করবে)।
  3. ডকার কন্টেইনারের লগ সাইজ লিমিট করতে `/etc/docker/daemon.json` ফাইলে লগ ড্রাইভারের ম্যাক্সিমাম সাইজ কনফিগার করুন:
     ```json
     {
       "log-driver": "json-file",
       "log-opts": {
     ...
     ```
     *তারপর ডকার সার্ভিস রিস্টার্ট করুন:* `systemctl restart docker`

---

### Q6: ডকার হোস্টে পোর্ট ৮০ খালি থাকা সত্ত্বেও কন্টেইনার রান করার সময় `port is already allocated` এরর দেখাচ্ছে। এই অর্ফ্যানড পোর্ট লকআপ কীভাবে রিলিজ করবেন?
* **সিনারিও:** কন্টেইনারটি স্টপ করার পরও হোস্টের পোর্টটি লক হয়ে আছে, ফলে নতুন কন্টেইনার রান করা যাচ্ছে না।
* **কারণ:** ডকার ডেমন যখন কোনো কন্টেইনার বন্ধ করে, তখন সে হোস্টের `iptables` ও ইন্টারনাল প্রক্সি ম্যাপিং থেকে পোর্ট রিলিজ করে দেয়। কিন্তু কার্নেলের নেটওয়ার্ক সকেট লকিং মেকানিজম বা ডকার ইন্টারনাল নেটওয়ার্ক ড্রাইভারের ডেডলকের কারণে মাঝে মাঝে পোর্টটি সঠিকভাবে আনলক হয় না (Orphaned Port Mapping)।
* **সমাধান:**
  1. পোর্টটি কোন প্রসেস হোল্ড করে রেখেছে তা খুঁজে বের করুন:
     `sudo lsof -i :80` অথবা `sudo netstat -lntp | grep 80`
  2. যদি প্রসেসটি ডকার প্রক্সি (`docker-proxy`) হয়, তবে কন্টেইনারের পোর্ট লক রিলিজ করতে ডকার ডেমন সার্ভিস রিস্টার্ট দিন:
     `sudo systemctl restart docker`
  3. প্রয়োজনে ফোর্সফুলি সেই নির্দিষ্ট প্রসেস আইডিটি কিল করুন: `kill -9 <PID>`

---

### Q7: কন্টেইনারের মেমরি গ্রাফে দেখা যাচ্ছে ব্যবহার ক্রমশ ১০০%-এর দিকে যাচ্ছে, কিন্তু কন্টেইনারে চলমান প্রসেসের রিয়েল মেমরি (RSS) অনেক কম। সার্ভার কি ক্র্যাশ করবে?
* **সিনারিও:** প্রমিথিউস বা গ্রাফানা মনিটরিংয়ে মেমরি অ্যালার্ট ট্রিগার হচ্ছে, কিন্তু অ্যাপ্লিকেশনে কোনো মেমরি লিক নেই।
* **কার্নেল লেভেল কারণ:** লিনাক্স কার্নেল সিস্টেমের অবিরত ডিস্ক রিড/রাইট অপ্টিমাইজ করার জন্য ফাঁকা মেমরিকে **Page Cache (Buffered Cache)** হিসেবে ব্যবহার করে। কার্নেল এই ক্যাশ মেমরিকে কন্টেইনারের cgroups লিমিটের আওতাভুক্ত করে। মনিটরিং টুলগুলো অনেক সময় `Cache Memory` কে `Active Memory` হিসেবে ভুলভাবে কাউন্ট করে গ্রাফ ১০০% দেখায়। বাস্তবে কার্নেল অত্যন্ত স্মার্ট; অ্যাপ্লিকেশনের যদি রিয়েল মেমরির (RSS) প্রয়োজন পড়ে, সে সেকেন্ডের ভেতর পেজ ক্যাশ রিলিজ করে অ্যাপকে মেমরি বুস্ট দেয়। এর জন্য কন্টেইনার ওএম কিল খাবে না।
* **ডায়াগনসিস:** 
  * কন্টেইনারের ভেতর মেমরি স্ট্যাটাস দেখুন: `cat /sys/fs/cgroup/memory/memory.stat | grep cache`
  * যদি `rss` ভ্যালু কম থাকে এবং `cache` বেশি থাকে, তবে চিন্তার কোনো কারণ নেই।

---

### Q8: প্রোডাকশনে একটি কন্টেইনার কম্প্রোমাইজড বা হ্যাক হওয়ার পর হ্যাকার হোস্টের রুট এক্সেস নিয়ে নিল। এই কন্টেইনার এস্কেপ কীভাবে প্রিভেন্ট করা যেত?
* **সিনারিও:** হ্যাকার কন্টেইনারের সিকিউরিটি হোল ভেঙে হোস্ট মেশিনের ফাইলসিস্টেমে রাইট পারমিশন নিয়ে নিয়েছে।
* **কারণ:** কন্টেইনারটি রান করার সময় অসাবধানতাবশত `--privileged` ফ্ল্যাগ দেওয়া হয়েছিল অথবা কন্টেইনারের ভেতরের প্রসেসটি `root` ইউজার হিসেবে চলছিল। `--privileged` ফ্ল্যাগ কন্টেইনারের আইসোলেশন দেওয়াল ভেঙে হোস্টের সমস্ত কার্নেল ক্যাপাবিলিটিজ কন্টেইনারকে দিয়ে দেয়।
* **সমাধান:**
  1. কন্টেইনার কখনো `--privileged` মোডে চালাবেন না।
  2. প্রসেসটি নন-রুট ইউজার হিসেবে রান করুন (Dockerfile-এ `USER node` বা কাস্টম ইউজার ডিফাইন করুন)।
  3. কন্টেইনারের রুট ফাইলসিস্টেম রিড-অনলি হিসেবে মাউন্ট করুন: `--read-only`
  4. কার্নেলের অতিরিক্ত পারমিশন ড্রপ করুন: `docker run --cap-drop=ALL --cap-add=NET_BIND_SERVICE my-image`

---

### Q9: কন্টেইনারের ভেতর থেকে বাইরের যেকোনো এপিআই (যেমন Stripe বা Payoneer) রিকোয়েস্ট পাঠানোর সময় ঘন ঘন DNS Resolution Timeout দেখাচ্ছে। এটি কীভাবে ফিক্স করবেন?
* **সিনারিও:** অ্যাপ্লিকেশন বাইরে কানেক্ট করতে পারছে না, কিন্তু পিন্গ করলে মাঝে মাঝে রেসপন্স পাওয়া যায়।
* **কারণ:** ডকার তার নিজস্ব ইন্টারনাল ডিএনএস সার্ভার (`127.0.0.11`) ব্যবহার করে কন্টেইনারগুলোর মধ্যে নাম রেজোলিউশন করার জন্য। কন্টেইনার যখন বাইরের কোনো অ্যাড্রেস রিকোয়েস্ট করে, তখন ডকার ডেমন হোস্টের `/etc/resolv.conf` ফাইলের ডিএনএস সার্ভারে সেটি ফরওয়ার্ড করে। হোস্ট যদি ডাইনামিক নেটওয়ার্ক বা লোকাল ভিপিএন ব্যবহার করে, তবে ডকারের ডিএনএস ক্যাশ ও লোকাল ডিএনএসের মধ্যে কানেক্টিভিটি লুপ বা টাইমআউট তৈরি হয়।
* **সমাধান:**
  1. কন্টেইনার চালুর সময় সরাসরি সিকিউর পাবলিক ডিএনএস ইনজেক্ট করে দিন:
     `docker run --dns=8.8.8.8 --dns=1.1.1.1 my-image`
  2. অথবা ডকার কম্পোজে গ্লোবাল ডিএনএস কনফিগার করুন:
     ```yaml
     services:
       my-service:
         dns:
...
### Q10: Dockerfile বিল্ড করার সময় `max depth exceeded` এরর দেখাচ্ছে। এর কারণ ও সমাধান কী?
* **সিনারিও:** আপনার ডকার ফাইলটি অনেক বড় এবং বিল্ড প্রসেস মাঝপথে বন্ধ হয়ে যাচ্ছে।
* **কারণ:** ডকার ইমেজ মূলত লেয়ার বাই লেয়ার (OverlayFS layers) তৈরি হয়। ডকার ফাইলে ব্যবহৃত প্রতিটা `RUN`, `COPY`, `ADD` কমান্ড একটি করে নতুন লেয়ার তৈরি করে। কার্নেলের Storage Driver (যেমন OverlayFS)-এর সর্বোচ্চ লেয়ার লিমিট ১২৮টি। ডকার ফাইলে অতিমাত্রায় সিঙ্গেল লাইন কমান্ড ব্যবহার করলে এই লিমিট ক্রস হয়ে বিল্ড ফেইল করে।
* **সমাধান:**
  1. একাধিক `RUN` কমান্ডকে চেইনিং মেথড এবং ব্যাকস্ল্যাশ (`\`) ব্যবহার করে একটি সিঙ্গেল লেয়ারে নিয়ে আসুন:
     ```dockerfile
     # খারাপ প্র্যাকটিস (৩টি লেয়ার তৈরি করবে)
     RUN apt-get update
     RUN apt-get install -y git
     RUN apt-get install -y curl
     
     # ভালো প্র্যাকটিস (১টি লেয়ারে কমপ্লিট)
     RUN apt-get update && apt-get install -y \
         git \
         curl \
         && rm -rf /var/lib/apt/lists/*
     ```
  2. মাল্টি-স্টেজ ডকার বিল্ড ব্যবহার করে ফাইনাল স্টেজকে একদম ক্লিন ও মিনিমাল লেয়ারে রাখুন।

---

### Q11: ডাটাবেস কন্টেইনারে ভলিউম মাউন্ট করার পর কন্টেইনারটি `Permission denied` এরর দিয়ে ক্র্যাশ করছে। কীভাবে হোস্ট ও কন্টেইনারের পারমিশন সিঙ্ক করবেন?
* **সিনারিও:** ডকার কম্পোজ আপ দেওয়ার পর Postgres বা MySQL কন্টেইনার ফাইলসিস্টেমে রাইট করতে পারছে না এবং ইমিডিয়েট বন্ধ হয়ে যাচ্ছে।
* **কারণ:** কন্টেইনারের ভেতরে ডাটাবেস সার্ভিসটি সাধারণত একটি নন-রুট সিকিউর সিস্টেম ইউজার (যেমন Postgres-এর UID `999`) দ্বারা চালিত হয়। আপনি যখন হোস্টের কোনো ডিরেক্টরি (যেমন `./pgdata`) কন্টেইনারে মাউন্ট করেন, হোস্টের সেই ডিরেক্টরিটির ওনারশিপ যদি রুট বা অন্য ইউজারের থাকে, তবে কন্টেইনারের ভেতরের নন-রুট ইউজারটি সেখানে রাইট করতে পারে না।
* **সমাধান:**
  1. হোস্টের মাউন্টেড ডিরেক্টরির ওনারশিপ কন্টেইনারের ইউজারের আইডি অনুযায়ী চেঞ্জ করে দিন:
     `sudo chown -R 999:999 ./pgdata`
  2. অথবা ডকার কম্পোজে ভলিউম ব্যবহারের ক্ষেত্রে ডকার ম্যানেজড ভলিউম (Named Volume) ব্যবহার করুন, যা পারমিশন অটোমেটিকালি হ্যান্ডেল করে।

---

### Q12: কন্টেইনারের ভেতর পাইথন বা পিএইচপি স্ক্রিপ্ট রান করার সময় `Bus error (coredump)` দিচ্ছে। মেমরি খালি থাকা সত্ত্বেও কেন এমন হচ্ছে?
* **সিনারিও:** মেমরি ও ডিস্ক স্পেস পর্যাপ্ত থাকা সত্ত্বেও ডাটা সায়েন্স (PyTorch/Tensorflow) বা ভারী কন্টেইনার ক্র্যাশ করছে।
* **কার্নেল লেভেল কারণ:** ডকার কন্টেইনারের ভেতরে শেয়ার্ড মেমরি স্পেস বা **`/dev/shm`** এর সাইজ ডিফল্টভাবে মাত্র **৬৪ মেগাবাইট (64MB)** সেট করা থাকে। প্রসেস যখন এর চেয়ে বেশি ডাটা শেয়ার্ড মেমরিতে রাইট করতে যায়, তখন কার্নেল সিগন্যাল দিয়ে প্রসেসটি কিল করে দেয় এবং বাস এরর জেনারেট করে।
* **সমাধান:**
  1. কন্টেইনার রান করার সময় `--shm-size` ফ্ল্যাগ দিয়ে শেয়ার্ড মেমরি সাইজ বাড়িয়ে দিন:
     `docker run --shm-size=2g -d my-deep-learning-image`
  2. ডকার কম্পোজ ফাইলে কাস্টম শেম সাইজ সেট করুন:
     ```yaml
     services:
       ai-service:
         shm_size: '2gb'
...
### Q13: `docker stop` কমান্ড দেওয়ার পর কন্টেইনার বন্ধ হতে ১০ সেকেন্ড সময় নিচ্ছে এবং ডাটাবেস কানেকশনগুলো ফোর্সফুলি কেটে যাচ্ছে। কীভাবে ইনস্ট্যান্ট গ্রেসফুল শাটডাউন করবেন?
* **সিনারিও:** বিল্ড পাইপলাইনে কন্টেইনার স্টপ হতে অনেক সময় নষ্ট হচ্ছে এবং ট্রাফিক ড্রপ খাচ্ছে।
* **কারণ:** `docker stop` কমান্ড দিলে ডকার ডেমন কন্টেইনারের মেইন প্রসেসকে (PID 1) প্রথমে **`SIGTERM`** সিগন্যাল পাঠায় গ্রেসফুলি কাজ শেষ করার জন্য। যদি PID 1 প্রসেসটি এই সিগন্যাল রিসিভ না করে বা ইগনোর করে, তবে ডকার ১০ সেকেন্ড অপেক্ষা করে প্রসেসটিকে **`SIGKILL`** সিগন্যাল দিয়ে জোরপূর্বক বন্ধ করে দেয়। নোড বা পাইথন স্ক্রিপ্ট সরাসরি রান করলে তারা অনেক সময় ওএসের সিগন্যাল হ্যান্ডেল করতে পারে না।
* **সমাধান:**
  1. আপনার অ্যাপ্লিকেশনের কোডে `SIGTERM` সিগন্যাল লিসেনার যুক্ত করুন এবং কানেকশন ক্লোজ করুন:
     ```javascript
     process.on('SIGTERM', () => {
       server.close(() => {
         console.log('Database and server closed gracefully');
         process.exit(0);
       });
     });
     ```
  2. ডকার ফাইলে `CMD node dist/main.js` এর পরিবর্তে `CMD ["node", "dist/main.js"]` (Exec Form) ব্যবহার করুন। প্রথম ফরম্যাটটি ব্যাকগ্রাউন্ডে শেলের ভেতর প্রসেসটি চালায়, ফলে সিগন্যাল লস্ট হয়ে যায়।

---

### Q14: কন্টেইনারের ভেতরের ঘড়ি লোকাল হোস্টের চেয়ে পিছিয়ে পড়েছে, ফলে Stripe/AWS এর JWT টোকেন এপিআই রিকোয়েস্ট রিজেক্ট হচ্ছে। এটি কীভাবে সিঙ্ক করবেন?
* **সিনারিও:** কন্টেইনার থেকে টাইমস্ট্যাম্প ম্যাচ না করার কারণে থার্ড পার্টি গেটওয়েতে রিকোয়েস্ট এরর দিচ্ছে।
* **কারণ:** ডকার কন্টেইনারের নিজস্ব কোনো ঘড়ি বা হার্ডওয়্যার ক্লক নেই। এটি সরাসরি হোস্ট ওএসের লিনাক্স কার্নেল ক্লক রিড করে। লোকাল মেশিনে (যেমন ল্যাপটপে) কাজ করার সময় ল্যাপটপ যখন স্লিপ মোডে যায়, তখন ডকার ডেমনের টাইমার সাময়িকভাবে থমকে দাঁড়ায় এবং রিয়েল টাইমের সাথে টাইমিং ড্রিফট (Time Drift) তৈরি হয়।
* **সমাধান:**
  1. আপনার হোস্টের ডকার এঞ্জিন রিস্টার্ট দিন:
     `sudo systemctl restart docker` (লোকাল ডকার ডেস্কটপ হলে ইউজার ইন্টারফেস থেকে রিস্টার্ট দিন)।
  2. প্রোডাকশন হোস্টে সর্বদাই **NTP (Network Time Protocol)** সচল রাখুন যাতে নোডের ঘড়ি নিখুঁত থাকে।

---

### Q15: সিকিউরিটি বাড়ানোর জন্য `--read-only` ফ্ল্যাগ দিয়ে কন্টেইনার চালানোর পর অ্যাপ্লিকেশন রাইট এরর দিয়ে ক্র্যাশ করছে। সমাধান কী?
* **সিনারিও:** কন্টেইনারের রুট ফাইলসিস্টেম রিড-অনলি করার পর প্রসেসটি কোনো সাময়িক লগ বা টেম্প ফাইল লিখতে পারছে না।
* **কারণ:** অ্যাপ্লিকেশন রান করার সময় কার্নেল ফাইলসিস্টেম লক করে দেওয়ায় কোনো ফিজিক্যাল ডিস্ক ব্লক রাইট করতে পারে না। কিন্তু নোড বা পাইথন অ্যাপ্লিকেশনের রানটাইমে সাময়িক টেম্পোরারি ফাইলের জন্য রাইট পারমিশন প্রয়োজন পড়ে।
* **সমাধান:**
  1. অ্যাপ্লিকেশনের প্রয়োজনীয় নির্দিষ্ট সাময়িক ফোল্ডারগুলোকে ডকারের ইন-মেমরি ফাইলসিস্টেম (**`tmpfs`**) দিয়ে মাউন্ট করে দিন:
     `docker run --read-only --tmpfs /tmp --tmpfs /var/run -d my-secure-app`
  2. এর ফলে রুট ফাইলসিস্টেম রিড-অনলি থাকলেও সাময়িক ফাইলগুলো র‍্যামের ভেতর রাইট হতে পারবে এবং সিকিউরিটিও বজায় থাকবে।

---

### Q16: ডকার কম্পোজে দুটি কন্টেইনার (API ও DB) চমৎকার চলছে। কিন্তু API কন্টেইনার থেকে ডাটাবেস কানেক্ট করতে গেলে `Connection refused` এরর দেখাচ্ছে। কেন এমন হয়?
* **সিনারিও:** লোকাল হোস্ট থেকে ডাটাবেসে কানেক্ট করা যাচ্ছে, কিন্তু কন্টেইনার নেটওয়ার্কে ব্যাকএন্ড ডাটাবেস খুঁজে পাচ্ছে না।
* **কারণ:** প্রতিটা ডকার কন্টেইনার নিজস্ব **Network Namespace** ও লুপব্যাক ইন্টারফেস ব্যবহার করে সম্পূর্ণ আলাদা আইপিতে চলে। আপনি যখন ব্যাকএন্ড কোডে হোস্ট হিসেবে `localhost` বা `127.0.0.1` লেখেন, তখন ব্যাকএন্ড কন্টেইনারটি নিজের ভেতরের লোকালহোস্টে ডাটাবেস খুঁজতে যায় যেখানে কোনো ডাটাবেস চলমান নেই।
* **সমাধান:**
  1. ডকার কম্পোজ নেটওয়ার্কের আন্ডারে কন্টেইনারের সার্ভিস নেমকে হোস্টনেম হিসেবে ব্যবহার করুন:
     `DATABASE_URL=postgresql://postgres:secure@postgres-db:5432/saas_db` (এখানে `postgres-db` হলো ডকার কম্পোজে ডাটাবেস সার্ভিসের নাম)।

---

### Q17: আপনার Dockerfile-এ প্রতিবার একটু কোড চেঞ্জ করলেও `npm install` রান হয়ে অনেক বেশি সময় নিচ্ছে। কীভাবে বিল্ড ক্যাশ অপ্টিমাইজ করবেন?
* **সিনারিও:** ডেভলপমেন্ট বা সিআই/সিডি পাইপলাইনে বিল্ড হতে ১০-১৫ মিনিট সময় লেগে যাচ্ছে।
* **কারণ:** ডকার ফাইল বিল্ড করার সময় উপর থেকে প্রতিটা লাইনের ক্যাশ মেমরি মিলায়। আপনি যদি প্যাকেজ ডিক্লেয়ার করার আগেই সম্পূর্ণ সোর্স কোড কপি করে ফেলেন (`COPY . .`), তবে কোডের একটা ডট চেঞ্জ হলেও ডকার ক্যাশ ইনভ্যালিড করে দেয় এবং পরবর্তী সমস্ত লাইনের বিল্ড স্ক্র্যাচ থেকে শুরু করে।
* **সমাধান:**
  1. প্যাকেজ ফাইলগুলো আগে কপি করে ডিপেন্ডেন্সি ইনস্টল করুন, তারপর সোর্স কোড কপি করুন:
     ```dockerfile
     # ১. প্যাকেজ ডেফিনিশন কপি করা
     COPY package*.json ./
     
     # ২. ডিপেন্ডেন্সি ইনস্টল (কোড চেঞ্জ হলেও এই লেয়ারটি ক্যাশ থেকে লোড হবে)
     RUN npm ci
     
     # ৩. ফাইনাল সোর্স কোড কপি
     COPY . .
     ```

---

### Q18: কন্টেইনারের ভেতর কোনো ইউজার ফর্ক বোম (Fork Bomb) রান করে সম্পূর্ণ হোস্ট সার্ভার ক্র্যাশ করে দিল। কার্নেল লেভেলে এটি কীভাবে প্রিভেন্ট করা যেত?
* **সিনারিও:** কন্টেইনারের কোডে রুট লুপ বা আরসিই অ্যাটাকের মাধ্যমে কন্টেইনারটি হোস্টের সমস্ত প্রসেস মেমরি ব্লক করে দিয়েছে।
* **কার্নেল লেভেল কারণ:** ডিফল্টভাবে ডকার কন্টেইনারের প্রসেস লিমিট আনলিমিটেড থাকে। কন্টেইনারের ভেতর কেউ যদি ফর্ক বোম (`:(){ :|:& };:`) রান করে, তবে কার্নেল একের পর এক প্রসেস তৈরি করতে করতে হোস্টের প্রসেস টেবিল এক্সহস্ট (exhaust) করে ফেলে, ফলে হোস্ট পুরো হ্যাং হয়ে যায়।
* **সমাধান:**
  1. cgroups v2 PID Controller ব্যবহার করে কন্টেইনারের সর্বোচ্চ প্রসেস আইডি তৈরির লিমিট বেধে দিন:
     `docker run --pids-limit 100 -d my-app`
  2. ডকার কম্পোজে প্রসেস লিমিট সেট করুন:
     ```yaml
     services:
       my-app:
         pids_limit: 100
...
### Q19: আপনার ব্যাকআপ কন্টেইনার হোস্ট ফাইলসিস্টেম থেকে ১ টেরাবাইট ফাইল রিড করায় হোস্টের ডাটাবেস কুয়েরি হঠাৎ স্লো হয়ে গেল। কার্নেল লেভেলে কী ঘটেছে এবং এর সমাধান কী?
* **সিনারিও:** ব্যাকআপ নেওয়ার সময় প্রোডাকশন ডাটাবেস ইউজারদের রিকোয়েস্টে টাইমআউট দিচ্ছে।
* **কার্নেল লেভেল কারণ:** ব্যাকআপ কন্টেইনার যখন হোস্টের ড্রাইভ থেকে বিশাল ফাইল রিড করে, কার্নেল সেই ফাইলগুলোকে মেমরি পেজ ক্যাশে (Page Cache) জমা করতে থাকে। এর ফলে কার্নেল হোস্টের রানিং ডাটাবেসের ঘন ঘন ব্যবহৃত ইন-মেমরি পেজ ক্যাশগুলোকে মেমরি থেকে বের (Evict) করে দেয়। পরবর্তীতে ডাটাবেস কুয়েরি রান করার সময় ডাটাবেসকে র‍্যামের পরিবর্তে সরাসরি ফিজিক্যাল ডিস্ক রিড করতে হয়, যা কুয়েরি পারফরম্যান্স ১০০ গুণ ধীর করে দেয়।
* **সমাধান:**
  1. ব্যাকআপ কন্টেইনারটি মেমরি লিমিটেড cgroup-এ চালান এবং কার্নেলকে বলুন এর রিড করা পেজ ক্যাশে না রাখতে।
  2. ব্যাকআপ স্ক্রিপ্টে ফাইল ওপেন করার সময় POSIX `fadvise` সিসকলের মাধ্যমে কার্নেলকে ইনস্ট্রাকশন দিন যে এই ডাটা পুনরায় লাগবে না:
     `posix_fadvise(fd, 0, 0, POSIX_FADV_DONTNEED);`

---

### Q20: কন্টেইনারের ভেতরের প্রসেস থেকে হোস্ট মেশিনের লোকালহোস্টে চালিত অন্য একটি সার্ভিসের এপিআই (`http://localhost:8000`) কানেক্ট করতে চাচ্ছেন। কীভাবে কানেক্ট করবেন?
* **সিনারিও:** কন্টেইনারের ভেতর `localhost` বা `127.0.0.1` দিলে কাজ করছে না।
* **কারণ:** কন্টেইনারের নিজস্ব নেটওয়ার্ক নেমস্পেস থাকায় `localhost` দিয়ে হোস্টের নেটওয়ার্ক পোর্ট এক্সেস করা সম্ভব নয়।
* **সমাধান:**
  1. ডকারের স্পেশাল হোস্ট গেটওয়ে ডিএনএস এলিয়াস ব্যবহার করুন:
     `http://host.docker.internal:8000` (এটি কন্টেইনার নেটওয়ার্কের গেটওয়ে হয়ে সরাসরি হোস্ট মেশিনের লোকালহোস্টে রিকোয়েস্ট রাউট করে)।
  2. কন্টেইনার রান করার সময় নিচের ডিরেক্টিভটি যুক্ত করুন:
     `--add-host=host.docker.internal:host-gateway` (যদি লিনাক্স হোস্টে লোকাল ডকার চলে)।
